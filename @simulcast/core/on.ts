			...
			return (handler: EventHandler<Events[Event]>) => {
				...
				return () => {
					const currentEventHandlers =
						getEventHandlers(eventRegistry);
					const filteredHandlers =
						filterHandler(currentEventHandlers);
					const setFilteredHandlers =
						setEventHandlers(filteredHandlers);
					const commitRemove = mutate(
						setFilteredHandlers(eventRegistry),
					);
